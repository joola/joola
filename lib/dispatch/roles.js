...
exports.list = {
  ...
  run: function (context, organization, callback) {
    ...
    joola.config.get('authentication:organizations:' + organization + ':roles', function (err, value) {
      /* istanbul ignore if */
      if (err)
        return callback(err);
...
exports.get = {
  ...
  run: function (context, organization, rolename, callback) {
    callback = callback || emptyfunc;
    joola.config.get('authentication:organizations:' + organization + ':roles:' + rolename, function (err, value) {
      /* istanbul ignore if */
      if (err)
        return callback(err);
...
exports.add = {
  ...
  inputs: ['organization', 'role'],
  _outputExample: {},
  _permission: ['manage_system'],
  _dispatch: {
    message: 'roles:add'
  },
  run: function (context, organization, role, callback) {
    callback = callback || emptyfunc;
    ...
    exports.get.run(context, organization, role.name, function (err, _role) {
      if (_role)
        return callback(new Error('Role [' + role.name + '] already exists for organization [' + organization + ']'));

      joola.config.set('authentication:organizations:' + organization + ':roles:' + role.name, role, function (err) {
        /* istanbul ignore if */
        if (err)
          return callback(err);
...
exports.update = {
  ...
  run: function (context, organization, role, callback) {
    ...
    exports.get.run(context, organization, role.name, function (err, value) {
      if (err)
        return callback(err);
...
exports.delete = {
  ...
  run: function (context, organization, role, callback) {
    ...
    exports.get.run(context, organization, role.name, function (err, value) {
      ...
      joola.config.clear('authentication:organizations:' + organization + ':roles:' + role.name, function (err) {
        /* istanbul ignore if */
        if (err)
          return callback(err);
