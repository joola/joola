...
exports.add = {
  ...
  run: function (context, workspace, role, callback) {
    ...
    exports.get.run(context, workspace, role.name, function (err, _role) {
      if (_role)
        return callback(new Error('Role [' + role.name + '] already exists for workspace [' + workspace + ']'));

      joola.config.set('workspaces:' + workspace + ':roles:' + role.name, role, function (err) {
        /* istanbul ignore if */
        if (err)
          return callback(err);
...
exports.update = {
  ...
  run: function (context, workspace, role, callback) {
    ...
    exports.get.run(context, workspace, role.name, function (err, value) {
      if (err)
        return callback(err);
...
exports.delete = {
  ...
  run: function (context, workspace, role, callback) {
    ...
    exports.get.run(context, workspace, role.name, function (err, value) {
      ...
      joola.config.clear('workspaces:' + workspace + ':roles:' + role.name, function (err) {
        /* istanbul ignore if */
        if (err)
          return callback(err);
