...
exports.fetch = {
  ...
  route: function (req, res) {
    ...
    joola.dispatch.emitWait('logger:fetch-request', {}, function (err, log) {
      if (err)
        return router.responseError(new router.ErrorTemplate('Failed to fetch logs: ' + (typeof(err) === 'object' ? err.message : err)), req, res);
      response = log;
  ...
  run: function (callback) {
    ...
    mongo.find('logger', 'events', {}, {limit: 1000}, function (err, logs) {
