 ...
 *  @description
 *  Uses joola.io [[dispatch-flow]].
 *
 *  This module is responsible for the entire flow relating to `datasources`, for example: listing or adding a datasource.
 *
 * - [list](#list)
 * - [add](#add)
 * - [update](#update)
 ...
 * @callback {Error} err Error if one occured, null if none.
 * @callback {Object} result JSON object of all available data sources.
 */
exports.list = {
 ...
 *
 * //client-side
 * joola.sdk.dispatch.datasources.add(options, function(err, result) {
 * 	console.log(err, result);
 * }
 *
 * //client-side using websocket
 * joola.sdk.io.socket.on('/datasources/add:done', function(result) {
 * 	console.log(result)
 * });
 * joola.sdk.io.socket.emit('/datasources/add', options);
 *
 * ```
 */
