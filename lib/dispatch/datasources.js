...
joola.dispatch.on('datasources', 'list-done', function (channel, ds) {
...
joola.dispatch.once('datasources', 'delete-request', function (channel, ds) {
  joola.logger.debug('Deleting datasource with the name ' + ds.name);
  return datasources.delete(ds);
});

joola.dispatch.on('datasources', 'delete-done', function (channel, ds) {
