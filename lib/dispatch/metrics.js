 ...
 *  @description
 *  joola.io uses the concept of `metric` to bind users together into a groups. metrics are logical business
...
exports.list = {
  ...
  run: function (context, workspace, collection, callback) {
    ...
    joola.dispatch.collections.get(context, workspace, collection, function (err, _collection) {
      ...
      return callback(null, _collection.metrics);
    });
  }
};
...
exports.patch = {
  ...
  run: function (context, workspace, collection, metric, payload, callback) {
    ...
    /*
     if (context.user.metric !== metric && !context.user.su) {
     var err = new Error('Forbidden');
     err.code = 403;
     return callback(err);
     }
