...
exports.list = {
  ...
  run: function (context, workspace, collection, callback) {
    ...
    var ensureRole = function (metrics) {
      var clean = [];
      metrics.forEach(function (m) {
        var hasRole = _.intersection(context.user.roles, m.roles);
        if (hasRole.length > 0)
          clean.push(m);
      });
      return clean;
    ...
    if (collection) {
      joola.dispatch.collections.get(context, workspace, collection, function (err, _collection) {
        ...
        return callback(null, ensureRole(joola.common.obj2array(_collection.metrics)));
      });
    }
    else {
      joola.dispatch.collections.list(context, workspace, function (err, collections) {
        ...
        async.parallel(calls, function (err, results) {
          ...
          return callback(null, ensureRole(metrics));
        });
      });
    }
  }
...
exports.get = {
  ...
  run: function (context, workspace, collection, key, callback) {
    ...
    joola.dispatch.collections.get(context, workspace, collection, function (err, _collection) {
      ...
      result = result[0];
      var hasRole = _.intersection(context.user.roles, result.roles);
      if (hasRole.length > 0)
