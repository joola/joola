...
manager.parseDimension = function (context, d, callback) {
  ...
  if (typeof d === 'object') {
    ...
    if (d.collection) {
      ...
      joola.dispatch.collections.list(context, context.user.workspace, function (err, collections) {
        if (err)
          return callback(err);
...
manager.buildMergeQueryPlan = function (collection, query, callback) {
  var plan = {
    uid: joola.common.uuid(),
    cost: 0,
    colQueries: {},
  ...
  query.metrics.forEach(function (metric) {
    ...
    if (!metric.formula && metric.collection) {
      ...
      colQuery.key = 'fixedkey'; //joola.common.hash(colQuery.type + '_' + metric.collection.id + '_' + JSON.stringify(_$match));
      if (colQuery.type == 'plain') {
        if (plan.colQueries[colQuery.key]) {
          _$group = joola.common.extend({}, plan.colQueries[colQuery.key].query[2].$group);
        }
        ...
        if (metric.aggregation == 'count')
          ...
          _$group[metric.key]['$' + (typeof metric.aggregation === 'undefined' ? 'sum' : metric.aggregation)] = '$' + metric.attribute.replace('.', '_');
        colQuery.query = [
          {$match: _$match},
          {$sort: {timestamp: -1}},
          // {$project: _$project},
...
manager.loadFromCache = function (context, query, callback) {
  ...
  try {
    ...
    var resultsQuery = {
      ...
      'timeframe.end': {$lt: query.timeframe.end}
    };
    joola.mongo.find('results', 'exchange', resultsQuery, {}, function (err, results) {
      if (err)
        return callback(err);
      ...
      if (results && results.length > 0) {
        ...
        results.forEach(function (result) {
          result.diff = query.timeframe.end - result.timeframe.end;
        });
        ...
        results[0].fromCache = true;
        joola.mongo.find('results', results[0]._key, {}, {}, function (err, cachedResults) {
          return callback(null, cachedResults, results[0]);
        });
...
manager.saveToCache = function (context, cachedMeta, cachedResults, results, timeframe, callback) {
  try {
    ...
    joola.mongo.insert('results', 'exchange', meta, {}, function (err) {
      ...
      if (results.documents.length > 0) {
        if (cachedMeta) {
          cachedResults.forEach(function (cr) {
            delete cr._id;
            results.documents.push(cr);
          });
        }
...
exports.stop = {
  ...
  _route: function (req, res) {
    ...
    joola.logger.debug('Stopping query [' + _params.querytoken + ']');
...
exports.fetch = {
  ...
  _route: function (req, res) {
    ...
    if (_params.options.realtime) {
      var realtimeUID = joola.common.uuid();
      ...
      var realtime = function () {
        request(false, function (err, result) {
          ...
          if (res.socket.disconnected || manager.runningQueries.indexOf(realtimeUID) == -1)
    ...
    manager.parse(context, options, function (err, query) {
      ...
      manager.applyFilters(query, function (err, query) {
        ...
        /*manager.loadFromCache(context, query, function (err, cachedResults, cachedMeta) {
         ...
         joola.logger.trace('Partial cached result found, completing the diff [' + (Math.round((cachedMeta.diff / 1000 / 60) * 100) / 100).toString() + ' min].');
         query.timeframe.start = cachedMeta.timeframe.end;
        ...
        manager.buildQueryPlan(query, function (err, queryplan) {
          ...
          manager.executePlan(context, queryplan, function (err, results) {
            ...
            /*
             ...
             joola.logger.trace('Merging cached results with [' + cachedResults.length + '] documents to new results containing [' + results.documents.length + '].');
             ...
             manager.saveToCache(context, cachedMeta, cachedResults, results, timeframe, function (err, meta) {
             manager.buildMergeQueryPlan(meta._key, query, function (err, queryplan) {
             if (err)
             return callback(err);
             manager.executePlan(context, queryplan, function (err, results) {
             if (err)
