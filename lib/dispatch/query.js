  ...
  query.dimensions.forEach(function (d, i) {
    if (d && typeof d === 'object') {
      col = _.find(joola.config.datamap.collections, function (collection) {
        return  collection.dimensions && collection.dimensions[d.id];
      });
      if (!col) {
        //metricsValid = false;
      }
      else {
        var dimension = joola.common.extend({}, col.dimensions[d.id]);
        query.dimensions[i] = joola.common.extend(dimension, d);
      }
    }
    else if (joola.config.datamap.dimensions[d]) {
  ...
  query.metrics.forEach(function (m, i) {
    ...
    if (m && typeof m === 'object') {
      if (!m.dependsOn && m.id)
        m.dependsOn = m.id;
      col = _.find(joola.config.datamap.collections, function (collection) {
        return  collection.metrics && collection.metrics[m.dependsOn];
      });
      if (!col) {
        col = _.find(joola.config.datamap.collections, function (collection) {
        ...
        var metric = joola.common.extend({}, col.metrics[m.dependsOn]);
        query.metrics[i] = joola.common.extend(metric, m);
        query.metrics[i].id = m.dependsOn;
        query.metrics[i].adhoc = true
        query.metrics[i].collection = col;
      }
    }
      ...
      if (col) {
        ...
        if (!m.id) {
          //metricsValid = false;
          query.metrics[i] = {};
          query.metrics[i].id = _m;
          query.metrics[i].name = _m;
          query.metrics[i].type = 'int';
          query.metrics[i].aggregation = 'sum';
          query.metrics[i].adhoc = true;

          console.log(query.metrics[i])
        }
        else {
          query.metrics[i] = m;
    ...
    results.documents.forEach(function (document, dindex) {
      ...
      results.metrics.forEach(function (metric) {
        ...
        if (metric.formula) {
          ...
          document.fvalues[metric.id] = document.values[metric.id].toString();
        }
  ...
  run: function (context, options, callback) {
    ...
    manager.parse(options, function (err, query) {
      if (err) {
        ...
        options.dimensions.forEach(function (d) {
          results.dimensions.push({
            id: d,
            name: d,
            type: d === 'timestamp' ? 'date' : 'string'
          });
        });
        options.metrics.forEach(function (m) {
          if (typeof m === 'object')
            results.metrics.push(m);
          else {
        ...
        return manager.formatResults(results, function (err, results) {
          if (err)
            return callback(err);
