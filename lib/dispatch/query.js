  ...
  query.dimensions.forEach(function (d, i) {
    if (d && typeof d === 'object') {
      ...
      if (!col) {
        ...
        var dimension = joola.common.extend({}, col.dimensions[d.key]);
    ...
    if (m && typeof m === 'object') {
      ...
      if (!col) {
        col = _.find(joola.config.datamap.collections, function (collection) {
          return collection.dimensions && collection.dimensions[m.dependsOn] && collection.dimensions[m.dependsOn];
        });
        if (!col) {
          metricsValid = false;
        }
          ...
          query.metrics[i] = joola.common.extend(dimension, m);
          query.metrics[i].dependsOn = m.dependsOn;
          query.metrics[i].attribute = m.dependsOn;
          query.metrics[i].adhoc = true;
          query.metrics[i].collection = col;
        }
      }
      else {
        var metric = joola.common.extend({}, col[m.dependsOn]);
        query.metrics[i] = joola.common.extend(metric, m);
        //query.metrics[i].fullkey = m.key + '_' + m.dependsOn;
        //query.metrics[i].key = m.dependsOn;
        query.metrics[i].attribute = m.dependsOn;
        query.metrics[i].adhoc = true;
        query.metrics[i].collection = col;
      }
      ...
      if (col) {
        query.metrics[i] = joola.common.extend({}, col[m]);
        query.metrics[i].attribute = m;
        query.metrics[i].adhoc = false;
        query.metrics[i].collection = col;
      }
      else {
        if (m.indexOf('.') > 0) {
          Object.keys(joola.config.authentication.organizations[context.user.organization].collections).forEach(function (key) {
            ...
            if (exists) {
              query.metrics[i] = joola.common.extend({}, exists);
              parts.shift();
              query.metrics[i].fullkey = parts.join('.');
              query.metrics[i].attribute = m;
              query.metrics[i].adhoc = false;
              query.metrics[i].aggregation = query.metrics[i].aggregation || 'sum';
              query.metrics[i].collection = collection;
            }
          });
        }
        else {
          ...
          //metricsValid = false;
          query.metrics[i] = {};
          query.metrics[i].key = m;
          query.metrics[i].attribute = m;
          query.metrics[i].name = m;
          query.metrics[i].datatype = 'int';
          query.metrics[i].aggregation = 'sum';
          query.metrics[i].adhoc = true;
        }
      }
    }
...
manager.buildQueryPlan = function (query, callback) {
  ...
  query.dimensions.forEach(function (dimension) {
    switch (dimension.datatype) {
      ...
      case 'ip':
      case 'string':
        $group._id[dimension.key] = '$' + dimension.key;
        break;
      case 'geo':
        break;
      default:
...
manager.formatResults = function (results, callback) {
  ...
  if (results.documents && results.documents.length > 0) {
    results.documents.forEach(function (document, dindex) {
      ...
      results.dimensions.forEach(function (dimension) {
        ...
        if (dimension.datatype == 'date') {
          ...
          document.fvalues[dimension.key] = joola.common.extend({ip: ip}, joola.common.geoip.lookup(ip));
        }
      ...
      results.metrics.forEach(function (metric) {
        if (metric.decimals) {
          document[metric.key] = Math.round(document[metric.key] * (Math.pow(10, metric.decimals))) / (Math.pow(10, metric.decimals));
        }
        else {
          try {
            document[metric.key] = Math.round(document[metric.key] * 100) / 100;
          }
          catch (ex) {
        ...
        if (document.values[metric.key]) {
          if (metric.prefix)
            document.fvalues[metric.key] = metric.prefix + document[metric.key].toString();
          if (metric.suffix)
            document.fvalues[metric.key] = document[metric.key].toString() + metric.suffix;
        }
        else
          document.fvalues[metric.key] = null;

        if (metric.datatype === 'dependent') {
    ...
    /*
     ...
     _templateItem.values[d.id] = null;
     _templateItem.fvalues[d.id] = null;
     });
     query.metrics.forEach(function (m) {
     _templateItem.values[m.id] = null;
     _templateItem.fvalues[m.id] = null;
     });
