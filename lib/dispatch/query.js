...
manager.buildQueryPlan = function (query, callback) {
  ...
  if (query.timeframe) {
    ...
    $match.timestamp = {$gte: query.timeframe.start.toISOString(), $lte: query.timeframe.end.toISOString()};
  }
...
manager.executePlan = function (context, queryplan, callback) {
  ...
  return async.map(arrCols, function iterator(_query, next) {
    var cursor = mongo.aggregate('cache', context.user.organization + '_' + _query.collection, _query.query, {explain: true, allowDiskUse: true, cursor: {batchSize: 100}}, function (err, results, e) {
