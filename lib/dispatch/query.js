  ...
  query.metrics.forEach(function (m, i) {
    if (typeof m === 'object') {
      ...
      if (col) {
        ...
        m = joola.config.datamap.metrics[m];
        if (!m)
          metricsValid = false;
        else {
          query.metrics[i] = m;
...
manager.formatResults = function (options, results, callback) {
  results.formattedDocuments = [];
  results.documents.forEach(function (document, dindex) {
    delete document.key;
    var fDocument = joola.common.extend({}, document);

    results.dimensions.forEach(function (dimension) {
      if (dimension.type === 'ip' && document[dimension.id] && document[dimension.id] !== '(not set)') {
        var ip = document[dimension.id];
        fDocument[dimension.id] = joola.common.extend({ip: ip}, joola.common.geoip.lookup(ip));
      }
      else if (dimension.type === 'geo') {

      }
      else {
        fDocument[dimension.id] = document[dimension.id].toString();
      }

      if (dimension.transform) {
        var transformFn = eval('(' + dimension.transform + ')');
        transformFn(dimension, fDocument[dimension.id]);
      }

      fDocument[dimension.name] = fDocument[dimension.id];
      delete fDocument[dimension.id];
    });
    results.metrics.forEach(function (metric) {
      if (metric.formula) {
        ...
        document[metric.id] = calc.apply(document, args);
        fDocument[metric.id] = document[metric.id];
      }
      if (document[metric.id]) {
        if (metric.prefix)
          fDocument[metric.id] = metric.prefix + document[metric.id].toString();
        if (metric.suffix)
          fDocument[metric.id] = document[metric.id].toString() + metric.suffix;
      }
      else
        fDocument[metric.id] = null;

      if (metric.type !== 'dependent')
        fDocument[metric.name] = fDocument[metric.id];
      delete fDocument[metric.id];
    });
    results.documents[dindex] = document;
    results.formattedDocuments.push(fDocument);
  });

  options.ts.end = new Date();
  options.ts.duration = options.ts.end.getTime() - options.ts.start.getTime();
  results.stats = {
    times: options.ts
  };
        ...
        manager.loadFromCache(query, function (err, cached) {
          ...
          manager.buildQueryPlan(query, function (err, queryplan) {
            ...
            manager.executePlan(queryplan, function (err, results) {
              if (err)
                return callback(err);
              manager.formatResults(options, results, function (err, results) {
                if (err)
                  return callback(err);
