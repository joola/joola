  ...
  results.documents.forEach(function (document, dindex) {
    ...
    results.dimensions.forEach(function (dimension) {
      ...
      if (dimension.type === 'ip' && document[dimension.id] && document[dimension.id] !== '(not set)') {
        ...
        document.fvalues[dimension.id] = document[dimension.id].toString();
      }
  ...
  _route: function (req, res) {
    ...
    var request = function (callback) {
      joola.dispatch.request(req.token._, 'query:fetch', _params, function (err, result) {
        ...
        if (err)
          if (callback)
            return callback(new exports.ErrorTemplate('Failed to route action [' + 'fetch' + ']: ' + (typeof(err) === 'object' ? err.message : err)));
          else
            return router.responseError(new exports.ErrorTemplate('Failed to route action [' + 'fetch' + ']: ' + (typeof(err) === 'object' ? err.message : err)), req, res);
      ...
      timerID = setTimeout(function () {
        if (callback)
          return callback(new exports.ErrorTemplate('Timeout while waiting for [' + 'fetch' + ']'));
        else
          return router.responseError(new exports.ErrorTemplate('Timeout while waiting for [' + 'fetch' + ']'), req, res);
      }, 5000);
    };
    ...
    if (_params.options.realtime) {
      ...
      var intervalID = setInterval(realtime, 1000);
    }
    request();
  },
