...
manager.executePlan = function (context, queryplan, callback) {
  ...
  return async.map(arrCols, function iterator(_query, next) {
    var storeName = 'cache';
    var collectionName = context.user.workspace + '_' + (queryplan.query._interval === 'day' ? 'ddate' : queryplan.query._interval) + '_' + _query.collection;
    if (queryplan.merge) {
      storeName = 'results';
      collectionName = queryplan.colQueries.fixedkey.collection;
    }
    console.log('running on ', collectionName);
