    ...
    var request = function (firstRun, callback) {
      ...
      joola.auth.extendToken(req.token, function (err, token) {
        ...
        timerID = setTimeout(function () {
          if (callback)
            return callback(500, new router.ErrorTemplate('Timeout while waiting for [' + 'fetch' + ']'));
          else
            return router.responseError(500, new router.ErrorTemplate('Timeout while waiting for [' + 'fetch' + ']'), req, res);
    ...
    if (_params.options.realtime) {
      if (typeof _params.options.realtime === 'boolean') {
        ...
        if (_params.options.realtime.interval < 1000)
          return router.responseError(500, new router.ErrorTemplate('Minimal realtime interval is 1000ms'));
      }
      else {
