...
exports.list = {
  ...
  run: function (context, workspace, callback) {
    ...
    if (!context.user.su && context.user.workspace != workspace)
...
exports.get = {
  ...
  run: function (context, workspace, username, callback) {
    ...
    joola.dispatch.workspaces.get(context, workspace, function (err, org) {
      /* istanbul ignore if */
      if (err)
        return callback(err);
      ...
      if (!found) {
        return callback(new Error('User [' + username + '] of workspace [' + workspace + '] does not exist.'));
      }
    });
  }
};
...
exports.update = {
  ...
  run: function (context, workspace, user, callback) {
    ...
    exports.get.run(context, workspace, user.username, function (err, _user) {
      ...
      //user = joola.common.extend(_user, user);
...
exports.delete = {
  ...
  run: function (context, workspace, user, callback) {
    ...
    exports.get.run(context, workspace, user.username, function (err, value) {
      if (err)
        return callback(err);

      joola.config.clear('workspaces:' + workspace + ':users:' + user.username, function (err) {
        /* istanbul ignore if */
        if (err)
          return callback(err);
        ...
        exports.get.run(context, workspace, user.username, function (err, value) {
          /* istanbul ignore if */
          if (value)
            return callback(new Error('Failed to delete user [' + user.username + '] of workspace [' + workspace + '].'));
  ...
  run: function (context, APIToken, callback) {
    ...
    joola.config.get('workspaces', function (err, orgs) {
      Object.keys(orgs).forEach(function (org) {
        var _org = orgs[org];
        if (_org.users) {
          Object.keys(_org.users).forEach(function (user) {
            user = _org.users[user];
            if (user._APIToken == APIToken) {
              _user = user;
              _user.workspace = org;
            }
          });
      ...
      if (_user) {
        return exports.generateToken.run(context, _user.workspace, _user, function (err, token) {
          _user.token = token;
