...
exports.add = {
  ...
  run: function (context, workspace, user, callback) {
    ...
    new Proto(context, workspace, user, function (err, user) {
      ...
      //check the creator does not extend a permission it does not hold
      user.permissions.forEach(function (permission) {
        if (context.user.permissions.indexOf(permission) === -1) {
          err = new Error('Forbidden');
          err.code = 403;
          return callback(err);
        }
        ...
        if (wrk.users) {
          Object.keys(wrk.users).forEach(function (user) {
            ...
            if (user.APIToken === APIToken) {
              found = true;
              return exports.get.run({user: joola.SYSTEM_USER}, wrk.key, user.username, callback);
            }
          });
        }
      });
...
exports.generateToken = {
  ...
  run: function (context, user, callback) {
    ...
    user.workspace = user.workspace || context.user.workspace;
    joola.auth.generateToken(user, function (err, token) {
      if (err)
        return callback(err);
...
exports.permissions = {
  ...
  run: function (context, workspace, user, callback) {
    ...
    var userPermissions = [];
    exports.get.run(context, workspace, user, function (err, _user) {
      if (err)
        return callback(err);

      return callback(null, _user.permissions);
    });
  }
};