...
joola.dispatch.once('users', 'add-request', function (err, user) {
  joola.logger.debug('New users request [' + user._username + ']');
...
joola.dispatch.once('users', 'add-done', function (err, user) {
  ...
  joola.logger.debug('New users added [' + user._username + ']');
});

joola.dispatch.once('users', 'delete', function (channel, user) {
  joola.logger.debug('Deleted users with the name ' + user._username);
});

joola.dispatch.on('users', 'update-request', function (channel, user) {
  joola.logger.debug('Update users with the name ' + user._username);
