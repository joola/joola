...
exports.getByToken = {
  ...
  dispatch: function () {
    var self = this;
    joola.dispatch.on('users:getByToken-request', function (channel, username) {
      joola.logger.debug('Getting user by token.');
      self.run(username, function (err, value) {
        if (err)
          return joola.dispatch.emit('users:getByToken-done', {err: err});

        joola.dispatch.emit('users:getByToken-done', {err: null, message: value});
      });
    });
  },
  route: function (req, res) {

    joola.dispatch.emitWait('users:getByToken-request', req.params.username, function (err, user) {
      if (err)
        return router.responseError(new router.ErrorTemplate('Failed to get user by token: ' + (typeof(err) === 'object' ? err.message : err)), req, res);
  ...
  run: function (username, callback) {
    callback = callback || emptyfunc;
    joola.auth.getUserByToken(username, callback);
  }
};
	...
	route: function (req, res) {
		try {
			var user = {
				...
				organization: req.params.user.organization
			};
...
exports.delete = {
	...
	dispatch: function () {
		...
		joola.dispatch.on('users:delete-request', function (channel, user) {
			joola.logger.debug('user delete request [' + user.username + ']');
			self.run(user, function (err, value) {
				if (err)
