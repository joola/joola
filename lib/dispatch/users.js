...
exports.get = {
  ...
  run: function (context, organization, username, callback) {
    callback = callback || emptyfunc;
    joola.config.get('authentication:organizations:' + organization + ':users:' + username, function (err, value) {
      try {
        if (err)
          return callback(err);
    ...
    new Proto(user, function (err, user) {
      ...
      joola.config.get('authentication:organizations:' + organization + ':users', function (err, value) {
        ...
        if (_users[user.username])
          return callback(new Error('A user with username [' + user.username + '] of orgnization [' + organization + '] already exists.'));
  ...
  run: function (context, organization, user, callback) {
    ...
    joola.config.get('authentication:organizations:' + organization + ':users', function (err, value) {
      ...
      if (!_users.hasOwnProperty(user.username))
        return callback(new Error('User with username [' + user.username + '] of organization [' + organization + '] does not exist.'));

      var _user = _users[user.username];
      _user = joola.common._extend(_user, user);
      _users[user.username] = _user;

      joola.config.set('authentication:organizations:' + organization + ':user:' + user.id, user, function (err) {
        if (err)
          return callback(err);
  ...
  run: function (context, organization, user, callback) {
    ...
    exports.get.run(context, user.username, function (err, value) {
      ...
      joola.config.clear('authentication:organizations:' + organization + ':users:' + user.username, function (err) {
        ...
        joola.config.get('authentication:organizations:' + organization + ':users:' + user.username, function (err, value) {
          if (err)
            return callback(err);
          if (!value)
            return callback(null, user);

          return callback(new Error('Failed to delete user [' + user.username + '] of organiztion [' + organization + '].'));
        });
      });
    });
  }
...
exports.verifyAPIToken = {
  ...
  run: function (context, APIToken, callback) {
    var _user;
    Object.keys(joola.config.authentication.organizations).forEach(function (org) {
      org = joola.config.authentication.organizations[org];
