...
exports.get = {
  ...
  run: function (context, workspaceid, callback) {
    callback = callback || function(){};

    joola.config.get('workspaces:' + workspaceid, function (err, value) {
      /* istanbul ignore if */
      if (err)
        return callback(err);

      if (typeof value === 'undefined')
        return callback(new Error('workspace with id [' + workspaceid + '] does not exist.'));
