 ...
 *
 * ```js
 * joola.dispatch.workspaces.get('test-org', function(err, result) {
 *   console.log(err, result);
 * }
 * ```
 */
exports.get = {
  ...
  description: "I get a specific workspace by name`",
  inputs: ['name'],
  _outputExample: {},
  _permission: ['manage_system'],
  _dispatch: {
  ...
  run: function (context, orgid, callback) {
    callback = callback || emptyfunc;
    
    joola.config.get('workspaces:' + orgid, function (err, value) {
      /* istanbul ignore if */
      if (err)
        return callback(err);

      if (typeof value === 'undefined')
        return callback(new Error('workspace with id [' + orgid + '] does not exist.'));
...
exports.add = {
  ...
  run: function (context, org, callback) {
    ...
    exports.get.run(context, org.id, function (err) {
      ...
      joola.config.set('workspaces:' + org.id, org, function (err) {
        /* istanbul ignore if */
        if (err)
          return callback(err);
...
exports.delete = {
  ...
  run: function (context, org, callback) {
    ...
    exports.get.run(context, org.id, function (err, value) {
      ...
      joola.config.clear('workspaces:' + org.id, function (err) {
        /* istanbul ignore if */
        if (err)
          return callback(err);
        ...
        exports.get.run(context, org.id, function (err, value) {
          ...
          return callback(new Error('Failed to delete workspace [' + org.id + '].'));
        });
      });
    });
  }
