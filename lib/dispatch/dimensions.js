 ...
 *  @description
 *  joola.io uses the concept of `dimension` to bind users together into a groups. dimensions are logical business
...
exports.list = {
  ...
  run: function (context, workspace, collection, callback) {
    ...
    joola.dispatch.collections.get(context, workspace, collection, function (err, _collection) {
      ...
      return callback(null, _collection.dimensions);
    });
  }
};
...
exports.patch = {
  ...
  run: function (context, workspace, collection, dimension, payload, callback) {
    ...
    /*
     if (context.user.dimension !== dimension && !context.user.su) {
     var err = new Error('Forbidden');
     err.code = 403;
     return callback(err);
     }
