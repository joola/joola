...
etl.crunch = function (context, workspace, collection, meta, documents, interval, options, callback) {
  ...
  crunched.forEach(function (cr) {
    ...
    traverse(meta).forEach(function (attribute) {
      if (attribute.type === 'metric') {
        var value = traverse(cr).get(attribute.path.split('.'));
        if (value && value[0]) {
...
etl.load = function (context, workspace, collection, documents, options, callback) {
  ...
  etl.furnish(context, workspace, collection, documents, function (err, col, meta) {
    ...
    joola.mongo.insert('cache', workspace + '_' + collection, ce.clone(documents), options, function (err, _documents) {
      ...
      //let's crunch!
      //etl.crunch(context, workspace, collection, meta, ce.clone(_documents), 'second', options, function (err) {
      //  if (err)
      //    console.log('crunch failed: second', err);
      etl.crunch(context, workspace, collection, meta, ce.clone(_documents), 'minute', options, function (err) {
        if (err)
          console.log('crunch failed: minute', err);
        //etl.crunch(context, workspace, collection, meta, ce.clone(_documents), 'hour', options, function (err) {
        // if (err)
        //    console.log('crunch failed: hour', err);
        //  etl.crunch(context, workspace, collection, meta, ce.clone(_documents), 'ddate', options, function (err) {
        //   if (err)
        //    console.log('crunch failed: ddate', err);
        //  etl.crunch(context, workspace, collection, meta, ce.clone(_documents), 'month', options, function (err) {
        //   if (err)
        //      console.log('crunch failed: month', err);
        //   etl.crunch(context, workspace, collection, meta, ce.clone(_documents), 'year', options, function (err) {
        //   if (err)
