...
etl.furnish = function (context, workspace, collection, documents, callback) {
  ...
  joola.dispatch.collections.metadata(context, workspace, ce.clone(documents[0]), collection, function (err, meta) {
    /* istanbul ignore if */
    if (err)
      return callback(err);
...
etl.crunch = function (context, workspace, collection, meta, documents, interval, options, callback) {
  ...
  documents.forEach(function (doc) {
    ...
    Object.keys(meta).forEach(function (key) {
      ...
      if (attribute.type !== 'metric' && ['_key', 'ourTimestamp', 'timestamp_timebucket'].indexOf(key) === -1)
    ...
    Object.keys(meta).forEach(function (key) {
      ...
      else if (key !== 'timestamp' && attribute.key)
  ...
  crunched.forEach(function (cr) {
    ...
    if (cr.saved) {
      joola.mongo.update('cache-' + interval, workspace + '_' + collection, filter, update, options, function (err) {
        expected--;
        if (expected === 0) {
...
etl.load = function (context, workspace, collection, documents, options, callback) {
  ...
  etl.furnish(context, workspace, collection, documents, function (err, col, meta) {
    ...
    joola.mongo.insert('cache', workspace + '_' + collection, ce.clone(documents), options, function (err, _documents) {
      ...
      //let's crunch!
      return etl.crunch(context, workspace, collection, meta, ce.clone(_documents), 'second', options, function () {
        etl.crunch(context, workspace, collection, meta, ce.clone(_documents), 'minute', options, function () {
          etl.crunch(context, workspace, collection, meta, ce.clone(_documents), 'hour', options, function () {
            etl.crunch(context, workspace, collection, meta, ce.clone(_documents), 'ddate', options, function () {
              etl.crunch(context, workspace, collection, meta, ce.clone(_documents), 'month', options, function () {
                etl.crunch(context, workspace, collection, meta, ce.clone(_documents), 'year', options, function () {
