...
etl.furnish = function (collection, documents, callback) {
  ...
  documents.forEach(function (document, index) {
    ...
    var documentKey = {}
    Object.keys(collection.dimensions).forEach(function (key) {
      var d = collection.dimensions[key];
      documentKey[d.id] = document[d.id];
    });
...
etl.load = function (collection, documents, options, callback) {
  ...
  etl.furnish(collection, documents, function (err) {
    ...
    joola.mongo.insert('cache', collection.id, documents, options, function (err) {
      return callback(err);
    });
  });
  //});
...
exports.insert = {
  ...
  _route: function (req, res) {
    ...
    exports.insert.run(null, _params.collection, _params.document, function (err, result) {
      if (err)
