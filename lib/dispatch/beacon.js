...
exports.insert = {
  ...
  _route: function (req, res) {
    ...
    try {
      ...
      joola.dispatch.request(_token._, 'beacon:insert', _params, function (err, result) {
        if (err)
  ...
  run: function (context, workspace, collection, documents, options, callback) {
    ...
    try {
      ...
      async.mapSeries(documents, function (d, next) {
        etl.verify(context, workspace, collection, d, function (err) {
          if (err)
            return setImmediate(function () {
              return next(err);
            });

          etl.load(context, workspace, collection, d, {}, function (err, document) {
            if (err)
              return setImmediate(function () {
                return next(err);
              });
        ...
        return setImmediate(function () {
          return callback(err, _documents);
        });
      });
    }
    catch (ex) {
