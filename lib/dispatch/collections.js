...
exports.list = {
  ...
  run: function (context, workspace, callback) {
    ...
    joola.config.get('workspaces:' + workspace + ':collections', function (err, collections) {
      /* istanbul ignore if */
      if (err)
        return callback(err);
  ...
  run: function (context, workspace, id, callback) {
    ...
    joola.config.get('workspaces:' + workspace + ':collections:' + id, function (err, value) {
      ...
      Object.keys(value).forEach(function (key) {
        var elem = value[key];
        if (elem.type === 'dimension') {
          value.dimensions.push(elem);
        }
        else if (elem.type === 'metric') {
          value.metrics.push(elem);
        }
      });
...
exports.patch = {
  ...
  run: function (context, workspace, collection, payload, callback) {
    ...
    exports.get.run(context, workspace, collection, function (err, value) {
      ...
      joola.config.set('workspaces:' + context.user.workspace + ':collections:' + collection, _collection, function (err) {
        /* istanbul ignore if */
        if (err)
          return callback(err);
