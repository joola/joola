...
exports.list = {
  ...
  run: function (context, workspace, callback) {
    ...
    joola.config.get('workspaces:' + workspace + ':collections', function (err, workspaces) {
      /* istanbul ignore if */
      if (err)
        return callback(err);

      var result = {};
      Object.keys(workspaces).forEach(function (wrk) {
        wrk = workspaces[wrk];
        console.log(wrk);
        if (wrk.strongTyped) {
          wrk.roles.read.forEach(function (role) {
            console.log(role);
            if (!result[wrk.key] && context.user._roles.indexOf(role) > -1) {
              result[wrk.key] = wrk;
            }
          });
        }
