...
exports.add = {
  ...
  run: function (context, workspace, collection, callback) {
    ...
    exports.get.run(context, workspace, collection.id, function (err, _collection) {
      if (_collection)
        return callback(new Error('Collection already exist'));

      joola.config.set('workspaces:' + workspace + ':collections:' + collection.id, collection, function (err) {
        /* istanbul ignore if */
        if (err)
          return callback(err);
...
exports.update = {
  ...
  run: function (context, workspace, collection, callback) {
    ...
    exports.get.run(context, workspace, collection.id, function (err) {
      if (err)
        return callback(err);

      joola.config.set('workspaces:' + context.user.workspace + ':collections:' + collection.id, collection, function (err) {
        /* istanbul ignore if */
        if (err)
          return callback(err);
    ...
    exports.get.run(context, workspace, id, function (err, value) {
      ...
      joola.config.clear('workspaces:' + workspace + ':collections:' + id, function (err) {
        ...
        exports.get.run(context, workspace, id, function (err, value) {
          ...
          joola.mongo.collection('cache', workspace + '_' + id, function (err, collection) {
            /* istanbul ignore if */
            if (err)
              return callback(err);
