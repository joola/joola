...
exports.add = {
  ...
  run: function (context, permission, callback) {
    ...
    exports.get.run(context, permission.name, function (err, _permission) {
      if (!err)
        return callback(new Error('Permission already exists'));

      joola.config.set('authentication:permissions:' + permission.name, permission, function (err) {
        /* istanbul ignore if */
        if (err)
          return callback(err);
