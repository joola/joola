...
dispatch.on = function (message, callback) {
  ...
  if (self.channels.indexOf(channel) == -1) {
    ...
    self.channels.push({key: joola.common.uuid(), channel: channel, cb: callback});
  }
...
dispatch.once = function (message, callback) {
  ...
  if (self.channels.indexOf(channel) == -1) {
    ...
    self.subscriber.subscribe(channel);
    self.channels.push({once: true, key: joola.common.uuid(), channel: channel, cb: function (err, message) {
