...
dispatch.init = function (callback) {
  try {
    ...
    self.auth = joola.config.get('store:dispatch:redis:auth');
    ...
    var stompSettings = {
      host: joola.config.get('store:dispatch:stomp:host') || '127.0.0.1',
      port: joola.config.get('store:dispatch:stomp:port') || 61613,
      user: joola.config.get('store:dispatch:stomp:user') || 'guest',
      password: joola.config.get('store:dispatch:stomp:pass') || 'guest',
      debug: false
    };
    ...
    var stompConnect = function () {
      ...
      dispatch.stompClient.on('disconnected', function (reason) {
        joola.logger.warn('STOMP disconnect, ' + (reason || 'no reason specified.'));
        if (!reconnectTimer) {
      ...
      dispatch.stompClient.on('closed', function (reason) {
        joola.logger.error('STOMP closed: ' + reason);
