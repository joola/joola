...
dispatch.init = function (callback) {
  ...
  dispatch.listen = function (client, destination, selector, messageProcessorFunction) {
    ...
    /*
     ...
     //console.log(util.format("listener.onMessage = %s", JSON.stringify(messageData)));
     ...
     //console.log(util.format("listener started message with routing key %s = %s", deliveryInfo.routingKey, messageBody));

     process.nextTick(function () {
     messageProcessorFunction(messageBody, messageHeaders, deliveryInfo, function (err, handlerMsg) {
     //console.log(util.format("listener finished message with routing key %s", deliveryInfo.routingKey));

     if (err) {
     var errMsg = util.format("listener handler returned error: %s", err.message);
     //console.log(errMsg);
     } else {
     //console.log(util.format("listener handler message returned: %s", handlerMsg));
     }
     });
     });
     });
     */
    //console.log('listener listening to ' + subscribeArgs.destination);
  ...
  dispatch.publishMessage = function (client, message, queueName, routingKey, replyTo) {
    ...
    //console.log(util.format("publishMessage %s", JSON.stringify(publisherArgs)));
  };
  ...
  var stompConnect = function () {
    ...
    dispatch.stompClient.on('connected', function () {
      //console.log(util.format("stomp.onConnected"));
      //listener(dispatch.stompClient, ['/queue/joola.dispatch'], "", dispatch.processMessage);
  ...
  joola.auth.getUserByToken(message.token, function (err, user) {
    ...
    try {
      ...
      listeners.forEach(function (listener) {
        ...
        //console.log('calling run', message, _params);
...
dispatch.fulfillRequest = function (message, headers) {
  ...
  var listeners = _.filter(dispatch.listeners, function (l) {
    //console.log(l._dispatch.message, headers.subscription.replace('/temp-queue/joola.dispatch.', '').replace('.', ':').replace('.done', ''))
    return l._dispatch.message === headers.subscription.replace('/temp-queue/joola.dispatch.', '').replace('.', ':').replace('.done', '');
  });
  //console.log(listeners);
  listeners.forEach(function (listener) {
    //console.log('looking for msg', message.id);
...
dispatch.request = function (token, channel, params, callback) {
  ...
  joola.mongo.insert('messages', 'router', compiled, {}, function (err) {
    ...
    if (typeof callback === 'function') {
      //console.log('set listener', compiled.id)
...
dispatch.fulfill = function (message, result, headers, callback) {
  //message = message[0];
  message = message;
  //console.log('full', typeof message, message);
  ...
  //console.log('publishing to', headers['reply-to'], message)
