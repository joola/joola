...
dispatch.request = function (channel, message, options, callback) {
  callback = callback || emptyfunc;
  joola.logger.silly('[dispatch] request for: ' + channel + ' [' + message.id + ']');

  dispatch.compose(channel, message, options, function (err, compiledMessage) {
    if (err)
      console.log(err);
...
dispatch.fulfill = function (message, payload, callback) {
  callback = callback || emptyfunc;
