...
dispatch.init = function (callback) {
  try {
    ...
    if (stompConfig && typeof stompConfig.enabled === 'undefined')
      stompConfig.enabled = true;
    if (stompConfig && stompConfig.enabled) {
      var dsn = stompConfig.dsn;
      if (dsn) {
        ...
        stompSettings.host = stompConfig.host || '127.0.0.1';
        stompSettings.port = stompConfig.port || 61613;
        stompSettings.user = stompConfig.user || 'guest';
        stompSettings.password = stompConfig.pass || 'guest';
        stompSettings.vhost = stompConfig.vhost || null;
      }
    ...
    joola.logger.debug('Connecting to Kafka server @ ' + joola.config.store.dispatch.zookeeper.connectionString);
    var client = new kafka.Client(joola.config.store.dispatch.zookeeper.connectionString, 'JOOLA-' + joola.UID),
      producer = new HighLevelProducer(client),
      consumer;

    var payloads = [
      {topic: '__heartbeat', messages: [new Date().getTime().toString()]}
    ];

    var groupId = 'joola' + joola.common.uuid(2);
    producer.on('ready', function () {
      var offset = new Offset(client);
      producer.createTopics(['__heartbeat'], function (err, data) {
        joola.logger.trace('Created [__heartbeat] topic.')
        console.log(groupId)
        ...
        /*consumer.on('message', function (message) {
          ...
          /*if (message.offset == 193) {
           console.log('setting offset');
           offset.commit('joola-', [
           {topic: message.topic, partition: message.partition.toString(), offset: parseInt(message.offset, 10)}
           ], function (err, data) {
           console.log(err);
           console.log(data);
           });
...
dispatch.processRequest = function (message, headers) {
  message = message[0];
  //message = jsonpack.unpack(message);
