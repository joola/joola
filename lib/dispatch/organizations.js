...
exports.get = {
  ...
  run: function (context, orgid, callback) {
    callback = callback || emptyfunc;
    joola.config.get('authentication:organizations:' + orgid, function (err, value) {
      if (err)
        return callback(err);

      if (typeof value === 'undefined')
        return callback(new Error('Organization with id [' + orgid + '] does not exist.'));
    ...
    exports.get.run(context, org.id, function (err, value) {
      ...
      joola.config.clear('authentication:organizations:' + org.id, function (err) {
        ...
        exports.get.run(context, org.id, function (err, value) {
          ...
          return callback(new Error('Failed to delete organization [' + org.id + '].'));
        });
      });
    });
  }
