...
etl.furnish = function (collection, document) {
  ...
  _.filter(collection.dimensions,function (dimension) {
    return dimension.type === 'date';
      ...
      var _date = new Date(document[dateDimension.name]);
      ...
      document[dateDimension.name + '_timebucket'] = bucket;
      document[dateDimension.name] = bucket.second;
    });
...
etl.load = function (collection, document, options, callback) {
  ...
  joola.mongo.insert('cache', collection.name, document, options, callback);
};
