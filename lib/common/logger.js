...
if (!joola.nolog) {
  joola.events.once('config:done', function () {
    ...
    try {
      var mongo_options = {
        ...
        host: joola.config.get('store:logger:mongo:host'),
        port: joola.config.get('store:logger:mongo:port'),
        user: joola.config.get('store:logger:mongo:user'),
        password: joola.config.get('store:logger:mongo:password'),
        db: joola.config.get('store:logger:mongo:db')
      };
  ...
  var logger = bunyan.createLogger({
    ...
    streams: [
      {
        stream: formatOut,
        level: joola.config.get('logger:console:level') || 'trace'
      },
      {
