...
if (!global.nolog) {
  joola.events.once('config:done', function () {
    ...
    try {
      ...
      try {
        logPath = '/var/log1/joola.io/log-' + process.pid + '.log';
        ...
        joola.logger.warn('File logging disabled. Could not resolve path to log file [' + '/var/log/joola.io/log-' + process.pid + '.log' + ']: ' + err.message);
      }
    }
  ...
  var logger = bunyan.createLogger({
    ...
    streams: [
      {
        stream: formatOut,
        level: joola.options.logger.level || 'trace'
      },
      {
