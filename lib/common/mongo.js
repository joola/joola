  ...
  if (!mongo.clients[store]) {
    ...
    mongo.stores[store].open(function (err, mongoClient) {
      ...
      try {
        ...
        //console.log('collection', collection, joola.config.store[store].mongo);
        db.createCollection(collection, joola.config.store[store].mongo.options || {}, function (err, collection) {
          ...
          collection.ensureIndex({_key: 1}, {unique: true}, function (err) {
            ...
            collection.ensureIndex({timestamp: 1}, {}, function (err) {
              ...
              if (joola.config.store[store].mongo.expireAfterSeconds) {
                collection.ensureIndex({ timestamp: 1 }, { expireAfterSeconds: joola.config.store[store].mongo.expireAfterSeconds }, function (err) {
                  if (err)
                    return callback(err);
