...
nconf.init = function (callback) {
  ...
  //we need to connect to redis, for this we need a the connection details.
  //first option, app arguments or enviornment variables.
  options.redis.host = nconf.get('stores:config:redis:host');
  options.redis.port = nconf.get('stores:config:redis:port');
  options.redis.DB = nconf.get('stores:config:redis:DB') || 0;
  ...
  if (options.redis.host == null || options.redis.port == null || options.redis.DB == null) {
    ...
    joola.logger.silly('Starting up with no env/arguments for redis config store, checking [' + configPath + '].');
    ...
    joola.logger.silly('Starting up with env/arguments for redis config store.');
  ...
  joola.config.get('version', function (err, value) {
    ...
    if (!value) {
      ...
      joola.logger.silly('Found a valid configuration store, version ' + value);
    }
