    ...
    joola.config.set = function (key, value, expire, callback) {
      ...
      joola.config.clear(key, function (err) {
        ...
        joola.config._set(key, value, function (err) {
          ...
          if (joola.dispatch)
            joola.dispatch.emitWait('config:change', {}, function () {
      ...
      Object.keys(rawConfig).forEach(function (key) {
        joola.config.set(key, rawConfig[key], function () {
          ...
          if (expected == counter) {
            ...
            var calls = [];
            joola.config.get('authentication:users', function (err, values) {
              Object.keys(values).forEach(function (user) {
                user = values[user];

                if (user.password == rawConfig['authentication'].users[user.username].password) {
                  var call = function (callback) {
                    joola.config.set('authentication:users:' + user.username + ':_password', joola.auth.hashPassword(user._password), callback);
                  };
                  calls.push(call);
                }
              });
