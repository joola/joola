...
var initial = true;
config.localApply = function (storeVersion, callback) {
  ...
  var configCopy = ce.clone(config._config);

  //first let's delete the existing workspaces config
  if (initial) {
    initial = false;
    delete configCopy.workspaces;
  }
    ...
    keys.forEach(function (key) {
      var call = function (callback) {
        joola.redis.get(key, function (err, value) {
          ...
          joola.common.flatGetSet(configCopy, key, value);
          return callback(null);
        });
      };
      calls.push(call);
