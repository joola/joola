...
function watch_state() {
  ...
  joola.dispatch.on('nodes:state:change', function (UID, state) {
    joola.logger.warn('Detected node [' + UID + '] has changed state to [' + state + ']');
  });

  setInterval(function () {
    var status = joola.state.get().status;
    if (status != lastStatus) {
      joola.logger.warn('Status change, joola.io is now [' + status + ']');
      lastStatus = status;
