...
state.set = function (control, state, details) {
  ...
  if (this.controls[control].state != state) {
    if (state != 'working' && state != 'online')
      ...
      joola.logger.info('State for [' + control + '] changed to [' + state + ']: ' + details);
    else
...
state.get = function () {
  ...
  Object.keys(state.controls).forEach(function (key) {
    //console.log(state.controls[key]);
    if (result.status != state.controls[key].state && state.controls[key].state != 'working' && state.controls[key].state != 'online') {
