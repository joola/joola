...
global.silentShutdown = function (code, callback) {
  ...
  stopped = true;
  joola.logger.debug('Gracefully shutting down, code: ' + code);
...
global.shutdown = function (code, callback) {
  console.log('Global Shutdown', code);
...
global.nodeState = function () {
  ...
  if (joola.webserver && joola.webserver.http)
    httpState = joola.webserver.http._handle?true:false;
  if (joola.webserver && joola.webserver.https)
    httpState = joola.webserver.https._handle?true:false;
