...
stats.queries.waittime = {
  ...
  lasthour: {
    ...
    query: [
      ...
      {$group: {_id: '$second', waittime: {$avg: '$waittime'}}},
      {$sort: {_id: -1}}
    ]
  }
};

stats.queries.querytime = {
  current: {
    name: 'querytime-current',
    collection: 'querytime',
    factor: 1,
    before: function (callback) {
      var $match = this[0].$match;
      var _date = new Date();
    ...
    query: [
      {$match: {timestamp: {$gt: 'LASTMINUTE'}}},
      {$sort: {timestamp: -1}},
      {$group: {_id: 'value', querytime: {$avg: '$metrics.querytime.value'}}}
    ]
  },
  lasthour: {
    name: 'querytime-lasthour',
    collection: 'querytime',
    factor: 60,
    before: function (callback) {
      var $match = this[0].$match;
    ...
    query: [
      {$match: {timestamp: {$gt: 'ANHOURAGO'}}},
      {$project: {second: '$timebucket.second', querytime: '$metrics.querytime.value'}},
      {$group: {_id: '$second', querytime: {$avg: '$querytime'}}},
      {$sort: {_id: -1}}
    ]
  }
};
