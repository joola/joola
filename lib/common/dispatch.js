        ...
        dispatch.listeners.forEach(function (listener) {
          if (listener.channel == channel) {
            ...
            if (listener.once && !listener.done) {
              listener.done = true;
              dispatch.removeListener(channel, callback);
...
dispatch.once = function (channel, callback) {
  ...
  if (!exist) {
    ...
    if (!dispatch.subscribed) {
      dispatch.subscribed = true;
      dispatch.subscriber.on('message', function (channel, message) {
        joola.logger.silly('[dispatch:once] received message on channel: ' + channel);
        dispatch.listeners.forEach(function (listener) {
          if (listener.channel == channel && listener.once === true && listener.done === false) {
            listener.done = true;
            joola.logger.silly('[dispatch] calling cb once: ' + channel);
            dispatch.removeListener(channel, callback);
            listener.cb(message);
          }
        });
...
dispatch.hook = function () {
  fs.readdirSync(path.join(__dirname, '../dispatch')).forEach(function (file) {
    if (file != 'index.js')
      return require('../dispatch/' + file);
  });
};