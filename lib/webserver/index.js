...
webserver.start = function (options, callback) {
  ...
  var
    ...
    redis = require('redis'),
    pub = redis.createClient(joola.config.get('store:websocket:http:redis:port') || 6379, joola.config.get('store:websocket:http:redis:host') || 'localhost', {return_buffers: true}),
    sub = redis.createClient(joola.config.get('store:websocket:http:redis:port') || 6379, joola.config.get('store:websocket:http:redis:host') || 'localhost', {return_buffers: true}),
    client = redis.createClient(joola.config.get('store:websocket:http:redis:port') || 6379, joola.config.get('store:websocket:http:redis:host') || 'localhost', {return_buffers: true});


  var
    secureRedisStore = require('socket.io-redis'),
    secureRedis = require('redis'),
    securePub = secureRedis.createClient(joola.config.get('store:websocket:https:redis:port') || 6379, joola.config.get('store:websocket:https:redis:host') || 'localhost', {return_buffers: true}),

    secureSub = secureRedis.createClient(joola.config.get('store:websocket:https:redis:port') || 6379, joola.config.get('store:websocket:https:redis:host') || 'localhost', {return_buffers: true}),
    secureClient = secureRedis.createClient(joola.config.get('store:websocket:https:redis:port') || 6379, joola.config.get('store:websocket:https:redis:host') || 'localhost', {return_buffers: true});
  ...
  if (redisPass) {
    pub.auth(redisPass, function (err) {
      if (err)
        return joola.logger.debug('Redis pass failed for IO-publisher: ' + err);
    });
    sub.auth(redisPass, function (err) {
      if (err)
        return joola.logger.debug('Redis pass failed for IO-subscriber: ' + err);
    });
    client.auth(redisPass, function (err) {
      if (err)
        return joola.logger.debug('Redis pass failed for IO-client: ' + err);
    });
  }
  ...
  if (db > 0) {
    pub.select(db, function (err) {
      if (err)
        return joola.logger.debug('Redis SELECT failed for IO-publisher: ' + err);

      return joola.logger.debug('Redis SELECT OK IO-publisher: ' + db);
    });
    sub.select(db, function (err) {
      if (err)
        return joola.logger.debug('Redis SELECT failed for IO-publisher: ' + err);

      return joola.logger.debug('Redis SELECT OK IO-subscriber: ' + db);
    });
    client.select(db, function (err) {
      if (err)
        return joola.logger.debug('Redis SELECT failed for IO-publisher: ' + err);

      return joola.logger.debug('Redis SELECT OK IO-client: ' + db);
    });
  }
  ...
  if (redisPass) {
    ...
    secureClient.auth(redisPass, function (err) {
      if (err)
        return joola.logger.debug('Redis pass failed for SIO-client: ' + err);
    });
  }
  ...
  if (secureDb > 0) {
    ...
    secureClient.select(secureDb, function (err) {
      if (err)
        return joola.logger.debug('Redis SELECT failed for SIO-publisher: ' + err);

      return joola.logger.debug('Redis SELECT OK SIO-client: ' + secureDb);
    });
  }
  ...
  var startHTTP = function (callback) {
    self.http = http.createServer(app).listen(self.options.port, self.options.bind || '0.0.0.0', function (err) {
      ...
      joola.io.adapter(secureRedisStore({key: 'clear', pubClient: securePub, subClient: secureSub}));
      joola.connectCounter = 0;
