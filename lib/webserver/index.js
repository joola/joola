...
webserver.start = function (options, callback) {
  ...
  var
    ...
    securePub = secureRedis.createClient(joola.config.get('store:websocket:https:redis:port') || 6379, joola.config.get('store:websocket:https:redis:host') || 'localhost'),
    secureSub = secureRedis.createClient(joola.config.get('store:websocket:https:redis:port') || 6379, joola.config.get('store:websocket:https:redis:host') || 'localhost'),
    secureClient = secureRedis.createClient(joola.config.get('store:websocket:https:redis:port') || 6379, joola.config.get('store:websocket:https:redis:host') || 'localhost');
  ...
  var handleSocketIOConnection = function (socket) {
    ...
    socket.on('message', function (event) {
      ...
      Object.keys(event).forEach(function (key) {
        args[0][key] = event[key];
        delete event[key];
      });
      event.args = args;
      event.name = event.args[0]._route;
  ...
  var startHTTPS = function (callback) {
    ...
    self.https = https.createServer(secureOptions, app).listen(self.options.secureport, self.options.bind || '0.0.0.0', function (err) {
      ...
      joola.sio = require('engine.io').attach(self.https);//secure_socketioWildcard(require('socket.io')).listen(self.https, {secure: true, log: true });
      ...
      /*joola.sio.enable('browser client minification');  // send minified client
       ...
       joola.sio.set('destroy upgrade', false);*/
      ...
      joola.sio.on('connection', handleSocketIOConnection);
