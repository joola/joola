...
webserver.start = function (options, callback) {
  ...
  if (db > 0) {
    securePub.select(db, function (err) {
      ...
      return joola.logger.debug('Redis SELECT OK SIO-publisher: ' + db);
    });
    secureSub.select(db, function (err) {
      if (err)
        return joola.logger.debug('Redis SELECT failed for SIO-publisher: ' + err);

      return joola.logger.debug('Redis SELECT OK SIO-subscriber: ' + db);
    });
    secureClient.select(db, function (err) {
      if (err)
        return joola.logger.debug('Redis SELECT failed for SIO-publisher: ' + err);

      return joola.logger.debug('Redis SELECT OK SIO-client: ' + db);
    });
  }
