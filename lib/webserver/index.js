...
webserver.start = function (options, callback) {
  ...
  self.options = {
    ...
    secure: true,
    secureOnly: false,
    keyFile: '/home/itay/certs/localhost.key',
    ...
    self.http = http.createServer(app).listen(self.options.port,function (err) {
      ...
      return callback(null, self.http);
        ...
        if (err.code != 'EADDRINUSE') {
          ...
          joola.logger.debug('[ignore] joola.io HTTPS server error: ' + err);
          joola.state.set('webserver-http', 'working', 'HTTP webserver-http disabled');
        }
      }).on('close', function () {
    ...
    self.https = https.createServer(secureOptions, app).listen(self.options.securePort,function (err) {
      ...
      return callback(null, self.https);
        ...
        if (err.code != 'EADDRINUSE') {
          ...
          joola.logger.debug('[ignore] joola.io HTTPS server error: ' + err);
          joola.state.set('webserver-https', 'working', 'HTTPS webserver-https disabled');
        }
      }).on('close', function () {
