...
webserver.stop = function (callback) {
  ...
  if (self.http) {
    joola.io.sockets.clients().forEach(function (socket) {
      socket.disconnect();
    });
    self.http.close();
  }
  if (self.https) {
    joola.sio.sockets.clients().forEach(function (socket) {
      socket.disconnect();
    });
    self.https.close();
  }
...
webserver.verify = function (callback) {
  var httpValid = this.options.secureOnly, httpsValid = this.options.secure;

  if (joola.state.controls['webserver-http'] && joola.state.controls['webserver-http'].state != 'working') {
    httpValid = false;
  }
  if (joola.state.controls['webserver-https'] && joola.state.controls['webserver-https'].state != 'working') {
    httpsValid = false;
  }
