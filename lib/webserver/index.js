...
webserver.start = function (options, callback) {
  ...
  if (secureSocketIORedisConfig.dsn) {
    ...
    socketIORedisOptions.https.port = parsed_url.port;
  ...
  var startHTTP = function (callback) {
    self.http = http.createServer(app).listen(self.options.port, self.options.bind || '0.0.0.0', function (err) {
      ...
      joola.logger.debug('joola HTTP server listening on port ' + self.options.port);
  ...
  var startHTTPS = function (callback) {
    ...
    self.https = https.createServer(secureOptions, app).listen(self.options.secureport, self.options.bind || '0.0.0.0', function (err) {
      ...
      joola.logger.debug('joola HTTPS server listening on port ' + self.options.secureport);
  ...
  if (!self.options.secureonly) {
    joola.logger.warn('Running HTTP. This is not recommended, read more at http://github.com/joola/joola/wiki.');
  ...
  if (self.options.secure) {
    ...
    joola.logger.warn('Running without HTTPS. This is not recommended, read more at http://github.com/joola/joola/wiki.');
  }
