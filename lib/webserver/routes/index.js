...
exports.setup = function (app) {
  ...
  //app.all('/:resource/:workspace/:action', require('../middleware/options').options);
  //app.all('/:resource/:action', auth.middleware, this.route);
  ...
  app.delete('/collections/:workspace/:key', middleware.middleware);

};

exports.router = function (endpoint) {
  return function (req, res, next) {
    console.log(endpoint, req.user, req.parsed);
    ...
    var dispatchRoute = function (req, res) {
      ...
      setImmediate(function () {
        ...
        joola.dispatch.request(req.token._, action._dispatch.message, req.parsed, function (err, result) {
          clearTimeout(timerID);
          if (aborted)
            return;
          if (err)
