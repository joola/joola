...
exports.login = function (req, res) {
  if (req.method == 'POST') {
    ...
    return res.render('login', {title: 'joola.io - Login'});
};
...
exports.setup = function (app) {
  ...
  app.get('/manage/:resource/:action?', auth.middleware, function (req, res) {
    if (req.params.action)
      res.render(req.params.resource + '/' + req.params.action);
    else
      res.render(req.params.resource);
  });
  app.get('/analytics/:resource/:action?', auth.middleware, function (req, res) {
    if (req.params.action)
      res.render(req.params.resource + '/' + req.params.action);
    else
      res.render(req.params.resource, {user: req.user, token: req.token._});
  });
};
