...
exports.responseSuccess = function (data, req, res) {
  ...
  if (req.start_ts) {
    joola.stats.set('waittime', {
      waittime$avg: new Date() - req.start_ts
    });
  }
    ...
    var dispatchRoute = function (req, res) {
      ...
      joola.dispatch.request(req.token._, action._dispatch.message, _params, function (err, result) {
        ...
        if (aborted)
          return;
        if (err)
          return exports.responseError(new exports.ErrorTemplate('Failed to route action [' + action.name + ']: ' + (typeof(err) === 'object' ? err.message : err)), req, res);
...
exports.login = function (req, res) {
  if (req.method == 'POST') {
    ...
    joola.dispatch.users.authenticate(null,req.body.email, req.body.password, function (err, user) {
      if (err)
