...
function build(callback) {
      ...
      collection.metrics.forEach(function (metric) {
        ...
        $li.on('click', function (e) {
          ...
          $placeholder.find('button.cog').on('click', function () {
            ...
            var config = {
              key: $placeholder.data('key') || collection.key + '.' + metric.key,
              name: $placeholder.data('name') || metric.name,
              aggregation: $placeholder.data('aggregation') || 'SUM',
              prefix: $placeholder.data('prefix') || metric.prefix,
              suffix: $placeholder.data('suffix') || metric.suffix
            };
      ...
      collection.dimensions.forEach(function (dimension) {
        ...
        $li.on('click', function (e) {
          ...
          $placeholder.find('button.cog').on('click', function () {
            ...
            var config = {
              key: $li.data('key') || collection.key + '.' + dimension.key,
              name: $li.data('name') || dimension.name,
              aggregation: $li.data('aggregation') || 'SUM',
              prefix: $li.data('prefix') || dimension.prefix,
              suffix: $li.data('suffix') || dimension.suffix
            };

            Object.keys(config).forEach(function (key) {
              $li.data(key, config[key]);
            });

            var options = {};
            $('#metricModal').attr('data-target', collection.key + '.' + dimension.key);
            $('#metricModal').modal(options);
            dSkipOne = true;
          });
          $placeholder.addClass('active');
  ...
  $('#metricModal .btnSubmit').on('click', function (e) {
    ...
    var $target = $('.metricpicker[data-member="' + $this.attr('data-target') + '"]');
