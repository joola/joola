...
joolaControllers.controller('settingsCtrl', function ($scope, settingsService) {
  ...
  $scope.settings.store.logger = {};
  $scope.settings.store.logger.mongo = {};
  ...
  settingsService.getSettings(function (err, res) {
    if (err)
      ...
      $scope.$apply(function () {
        $scope.settings.interfaces = res.interfaces;
        $scope.settings.store = res.store;
        $scope.settings.general = res.general;
      });
    }
  });
  ...
  $scope.saveSettings = function () {
    ...
    if (validateForm('systemsettings', validate)) {
      var settings = null;
      settingsService.updateStore($scope.settings.store, function (err) {
        if (err)
          console.log('Error!', err);
        else {
          settingsService.updateGeneral($scope.settings.general, function (err) {
