...
joolaControllers.controller('settingsCtrl', function ($scope, settingsService) {
  ...
  $scope.settings.store.beacon = {};
  $scope.settings.store.cache = {};
  $scope.settings.store.dispatch = {};
  $scope.settings.store.config = {};
  $scope.settings.store.runtime = {};
  $scope.settings.store.socketio = {};
  $scope.settings.store.stats = {};
  $scope.settings.logging = {};
  ...
  settingsService.getSettings(function (err, res) {
    if (err)
      ...
      $scope.$apply(function () {
        ...
        $scope.settings.interfaces.webserver.enabled = res.interfaces.webserver.enabled;
        $scope.settings.interfaces.webserver.secure = res.interfaces.webserver.secure;
        $scope.settings.interfaces.webserver.securePort = res.interfaces.webserver.securePort;
        $scope.settings.interfaces.webserver.sessionDuration = res.interfaces.webserver.sessionDuration;
        $scope.settings.interfaces.webserver.keyFile = res.interfaces.webserver.keyFile;
        $scope.settings.interfaces.webserver.certFile = res.interfaces.webserver.certFile;
        $scope.settings.interfaces.beacon.enabled = res.interfaces.beacon.enabled;
        $scope.settings.interfaces.beacon.port = res.interfaces.beacon.port;
        $scope.settings.interfaces.repl.enabled = res.interfaces.repl.enabled;
        $scope.settings.interfaces.repl.port = res.interfaces.repl.port;
        $scope.settings.logging.level = res.logging.level;
        $scope.settings.logging.stores = res.logging.stores;
      });
    }
  });
  ...
  $scope.saveSettings = function () {
    ...
    if (validateForm('systemsettings', validate)) {
      var settings = null;
      settingsService.updateSettings($scope.settings, function () {
        console.log('updated settings successfuly', $scope.settings);
      });
    }
  }
  ...
  $scope.saveInterfaces = function () {
    settingsService.updateInterfaces($scope.settings.interfaces, function(err, res) {
      if (err)
        console.log('Error! ', err)
      else
