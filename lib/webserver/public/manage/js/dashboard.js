...
function getBaseline() {
  ...
  return [
    {data: seriesA, yaxis: 1, hoverable: true, lines: { show: true, fill: true }},
    {data: seriesB, yaxis: 2, hoverable: true, bars: { show: true } }
  ];
}
...
joolaio.io.socket.on('stats:events', function (message) {
  ...
  var cpuAvg = cpuSum / i;
  cpuAvg = Math.round(cpu[cpu.length-1][1] * 10) / 10;
  ...
  var data = [
    {data: cpu, yaxis: 1, hoverable: true, lines: { show: true, fill: true }},
    {data: mem, yaxis: 2, hoverable: true, bars: { show: true }}
  ];
