	...
	joolaio.services = {
		start: function (name, options, callback) {
			...
			var spawnOptions = {
				...
				'logFile': __dirname + '/../logs/log-' + name + '.log',
				'outFile': __dirname + '/../logs/out-' + name + '.log',
				'errFile': __dirname + '/../logs/err-' + name + '.log'
			};
			...
			if (options.verify) {
				require('sleep').sleep(options.wait || 5);
				//joolaio.log.debug('Calling control-port @ ' + 'http://localhost:' + options.controlport);
				var request = require('supertest')('http://localhost:' + options.controlport);
				request.get('/status').expect(200).end(function (err, res) {
					if (err)
						return callback(err);

					if (res.body.status.toLowerCase() == 'running') {
						if (!failureReported) {
						...
						if (!failureReported) {
							failureReported = true;
							return callback(new Error('Failed to verify service: ' + res.body.status));
						}
					}
