...
joola.config.init(function (err) {
  ...
  joola.dispatch.init(function (err) {
    ...
    joola.caching.init(joola.config.store.cache.redis, function (err) {
      if (err) {
        return joola.logger.error('Failed to initialize caching: ' + err);
      }
      joola.logger.info('Caching initialized');
    });
