...
auth.validateAction = function (action, req, res, callback) {
	...
	required.forEach(function (param) {
		if (!req.params[param])
			return callback(new auth.AuthErrorTemplate('Required param [' + param + '] is missing.'), false);
	...
	if (req.user._roles) {
		req.user._roles.forEach(function (role) {
			joola.config.authentication.roles.forEach(function (roleToCompareAgainst) {
	...
	if (joola.common.isArray(action._permission))
		action._permission = action._permission[0];
	if (userPermissions.indexOf(action._permission) == -1)
...
auth.middleware = function (req, res, next) {
	...
	exports.validateToken(token, function (err, _token) {
		if (_token) {
			...
			joola.logger.silly('Token [' + token._ + '] is valid for user [' + token.user.username + '].');
		}
		else {
