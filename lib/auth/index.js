...
auth.generateToken = function (req, user, callback) {
  ...
  new UserProto(joola.ROOT, user.workspace, user, function (err, user) {
    if (err)
      return callback(err);
    token.user = user;
...
auth.validateToken = function (req, token, APIToken, callback) {
  ...
  if (APIToken) {
    ...
    if (token == joola.config.get('authentication:bypassToken')) {
      var bypassUser = {
        username: 'bypass',
        _roles: ['root', 'user'],
        workspace: 'root'
      };
      joola.logger.trace({category: 'security'}, 'Applying bypass token for request [1].');
...
auth.validateAction = function (action, req, res, callback) {
  ...
  if (!req.method)
    req.method = 'GET';
  if (req.method.toUpperCase() != (action.verb || 'GET')) {
  ...
  if (req.user._roles) {
    joola.workspaces.get({user: req.user}, req.user.workspace, function (err, workspace) {
      if (err) {
        joola.logger.warn({category: 'security'}, 'Missing role [' + req.user._roles.join(',') + '] for user ' + req.user.username + ', with workspace [' + req.user.workspace + ']');
        return process.nextTick(function () {
...
auth.checkRateLimits = function (req, res, callback) {
  req.limits = {
    limit: req.user.rateLimit || 5000,
    remaining: 0,
    reset: 0
  };

  var redisKey = 'rateLimits:' + req.user.workspace + ':' + req.user.username;
  joola.redis.incr('rateLimits:' + req.user.workspace + ':' + req.user.username, function (err, result) {
    if (err)
      return callback(err);
...
auth.middleware = function (req, res, next) {
  ...
  if (!token && !APIToken && parts.pathname != '/logout') {
    ...
    return router.responseError(401, err, req, res);
  }
  ...
  exports.validateToken(req, token, APIToken, function (err, _token) {
    if (_token) {
      ...
      joola.logger.debug({category: 'security'}, 'Token [' + _token + '] is invalid.');
    ...
    exports.checkRateLimits(req, res, function (err) {
      if (err) {
        err.help_url = 'http://github.com/joola/joola.io/wiki/rate-limits';
