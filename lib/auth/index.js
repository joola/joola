...
auth.middleware = function (req, res, next) {
	...
	//TODO: FIX THIS
	//if (!joola.config.authentication)
	//	return setTimeout(auth.middleware(req, res, next),500);
	...
	exports.validateToken(token, function (err, _token) {
		if (_token) {
			...
			joola.logger.silly('Token [' + token._ + '] is valid for user [' + _token.user.username + '].');
		}
		else {
