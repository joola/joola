...
auth.extendToken = function (token, callback) {
  ...
  joola.redis.expire('auth:tokens:' + token._, expiration, function (err, a) {
    if (err)
      joola.logger.warn('[auth] failed extending security token for [' + token.user.username + ']: ' + err);
    else
      joola.logger.trace('[auth] extended security token for [' + token.user.username + ']');
...
auth.generateToken = function (user, callback) {
  ...
  joola.redis.hmset('auth:tokens:' + token._, token, function (err) {
    ...
    joola.logger.trace('[auth] generated token [' + token._ + '] for [' + token.user.username + '].');
 ...
 */
auth.validateToken = function (token, APIToken, callback) {
  callback = callback || emptyfunc;
  if (!token && !APIToken)
    return callback(new Error('Missing token for validation'));

  if (APIToken) {
    joola.logger.trace('Verifying API Token [' + APIToken + ']');
    ...
    if (token == joola.config.authentication.bypassToken) {
      ...
      joola.logger.trace('Applying bypass token for request [1].');
...
auth.validateRoute = function (modulename, action, callback) {
  ...
  try {
    ...
    joola.logger.trace('Failed to validate route for [' + modulename + '][' + action + ']');
    return callback(ex);
  }
  ...
  try {
    ...
    joola.logger.trace('Validated route for [' + modulename + '][' + action + ']');
...
auth.validateAction = function (action, req, res, callback) {
  ...
  if (req.user._roles) {
    req.user._roles.forEach(function (role) {
      if (joola.config.workspaces[req.user.workspace]) {
        Object.keys(joola.config.workspaces[req.user.workspace].roles).forEach(function (roleToCompareAgainst) {
          if (role === roleToCompareAgainst) {
            userPermissions = userPermissions.concat(joola.config.workspaces[req.user.workspace].roles[roleToCompareAgainst].permissions);
          }
        });
      }
      else {
        joola.logger.warn('Missing role [' + req.user._roles.join(',') + '] for user ' + req.user.username + ', with workspace [' + req.user.workspace + ']');
      }
    });
  }
...
auth.middleware = function (req, res, next) {
  ...
  joola.logger.trace('Authentication request [' + parts.pathname + ']');
  ...
  var APIToken = req.headers['joolaio-apitoken'];
  ...
  exports.validateToken(token, APIToken, function (err, _token) {
    if (_token) {
      //joola.stats.incr('auth:tokens:validated');
      joola.logger.debug({category: 'security', req: req}, 'Token [' + _token._ + '] is valid for user [' + _token.user.username + '].');
    }
    else {
      //joola.stats.incr('auth:tokens:invalid');
      //we have invalid token, throw an exception
      joola.logger.info({category: 'security', req: req}, 'Token [' + _token + '] is invalid.');
...
auth.getUserByToken = function (token, callback) {
  ...
  if (token == joola.config.authentication.bypassToken) {
    ...
    joola.logger.trace('Applying bypass token for request [2].');
